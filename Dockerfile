...
LABEL "maintainer"="AbdelMoneim Azzaz <aazzaz@sbm.com.sa>" \
...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 COPY ./11.0.0.3/config/*.sh /usr/local/bin/
...
RUN useradd --create-home --home-dir /home/aceuser -G mqbrkrs,sudo,mqm aceuser \
  ...
  && su - aceuser -c '. /opt/ibm/ace-11.0.0.3/server/bin/mqsiprofile && mqsicreateworkdir /home/aceuser/ace-server' \
...
COPY --chown=aceuser ./11.0.0.3/binary/$BAR1 /tmp
...
COPY ./11.0.0.3/config/server.conf.yaml /home/aceuser/ace-server/
